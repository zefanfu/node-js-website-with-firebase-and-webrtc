(function(){
	...
	app.controller('RemoteStreamsController', ['camera', '$location', '$http', function(camera, $location, $http){
		...
		rtc.call = function(stream){
			...
			**/
			...
			if(camera.isOn){
				...
				if(stream.isPlaying){
					client.peerRenegociate(stream.id);//offer
				} else {

					client.peerInit(stream.id);//init
				}
    	...
    	if(url.indexOf('signedin') != -1 ){
    		var uid = $location.path().split("/")[3];
            client.send('idChange', { idContent: uid });
            client.setId(uid);
    	}
    	else{
		...
		localStream.toggleCam = function(){
			if(localStream.cameraIsOn){
				...
				.then(function(result) {
                    ...
                    if(newurl.indexOf('signedin') != -1 ){
                        var uid = newurl.split("/")[3];
                        localStream.name=newurl.split("/")[2];
                        client.send('idChange', { idContent: uid });
                        client.setId(uid);
                        localStream.link = $window.location.host + '/' + uid;
                    }
